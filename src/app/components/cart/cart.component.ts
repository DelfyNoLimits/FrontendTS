...
export class CartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (cartId) {
      this.http.get(`http://localhost:8080/api/tripstore/v1/cart-items/shopping-carts/${cartId}`)
  ...
  removeFromCart(item: any) {
    ...
    this.http.delete(`http://localhost:8080/api/tripstore/v1/cart-items/${cartItemId}`)
      .subscribe(() => {
  ...
  loadShoppingCart() {
    ...
    if (cartId) {
      this.http.get(`http://localhost:8080/api/tripstore/v1/cart-items`)
        .subscribe((response: Object) => {
          this.shoppingCart = response as any[];
        });
    }
