...
export class ProductComponent implements OnInit, AfterViewInit {
  ...
  checkExistingCart(userId: string): Observable<boolean> {
    const url = `http://localhost:8080/api/tripstore/v1/shopping-carts`;
  ...
  createCart(userId: string): Observable<any> {
    const createUrl = 'http://localhost:8080/api/tripstore/v1/shopping-carts';
    const getAllUrl = 'http://localhost:8080/api/tripstore/v1/shopping-carts';
  ...
  addToCart(product: any) {
    ...
    if (userId) {
      this.checkExistingCart(userId).subscribe((cartExists: boolean) => {
            ...
            const url = 'http://localhost:8080/api/tripstore/v1/cart-items';
